...
client.on("message", async (message) => {
  ...
  if (message.content.startsWith(`${prefix}naxi`)) {
    ...
    if (radio_name !== "" && naxi.includes(radio_name)) {
      ...
      Playing(
        `NAXI ${rest}`,
        message,
        `http://naxidigital-${radio_name}128.streaming.rs:8${radio_number}0/;*.mp3`
      );
      ...
      Playing(
        "NAXI",
        message,
        "http://naxidigital-house128.streaming.rs:8000/;*.mp3"
      );
    }
  }
  ...
  if (message.content.startsWith(`${prefix}antenaZg`)) {
    ...
    Playing(
      "Antena Zagreb",
      message,
      "http://live.antenazagreb.hr:8002/;*.mp3"
    );
  }
  ...
  if (message.content.startsWith(`${prefix}otvorenaMreza`)) {
    ...
    Playing(
      "Otvorena Mreza",
      message,
      "http://s8.iqstreaming.com:8050/stream;"
    );
  }
